...
func _send(delta: float) -> void:
	...
	if _send_timer < 1.0 / send_rate:
		return
	_send_timer = 0.0
	_receive_state.rpc(character.global_position, character.rotation.y, character.velocity, character.state, character.running, root_transform())
...
func _receive_state(position: Vector3, yaw: float, velocity: Vector3, state: int, running: bool, root: Transform3D) -> void:
